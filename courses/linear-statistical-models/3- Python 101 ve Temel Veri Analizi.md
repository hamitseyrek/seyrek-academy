...
- Jupyter Notebook: hücreler ve `kernel` mantığı
- Python temel söz dizimi: sayılar, metinler, tipler
- Listeler ve `for` döngüsü
- Koşullar: `if`–`elif`–`else`
- Sözlükler: anahtar–değer yapısı
- `pandas.DataFrame`: `head`, `info`, `describe`
- Kolon seçimi, filtreleme ve türetilmiş sütunlar
- Minik analiz senaryosu: özet istatistik + filtre
- Fonksiyonlara giriş ve küçük simülasyonlar (ek bölümler)
